...
- **WHEN** `slotIndex === -1`, `G.pearlDeck.length` wird nach `pop()` zu `0`, `G.pearlDiscardPile.length > 0`, und `G.pearlSlots.length === 4`
...
- **WHEN** `slotIndex === -1`, `G.pearlDeck.length` wird nach `pop()` zu `0`, `G.pearlDiscardPile.length === 0`
