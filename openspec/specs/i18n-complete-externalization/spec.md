# i18n Complete Externalization

## Overview

All user-visible strings in `game-web/src` must be externalized into the central translation file. No hardcoded text is permitted in JSX, Canvas `fillText` calls, or lib files. This covers component dialogs, canvas labels, ability texts in `abilityDisplayMap.ts`, and session info texts in `LobbyScreen`.
